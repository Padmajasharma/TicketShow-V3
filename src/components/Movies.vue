  ...
  <div class="page">
    ...
    <div class="container">
      <header class="page-header">
        ...
        <p class="page-subtitle">Discover the latest films available for booking.</p>
      </header>
      ...
      <div v-else-if="error" class="state state-error">
        <p>⚠️ Failed to load movies. {{ error }}</p>
        <button class="retry-btn" @click="fetchMovies">Try Again</button>
      </div>
      <div v-else-if="movies.length === 0" class="state state-empty">
        <p>No movies found at the moment. Check back soon!</p>
      </div>
      ...
      <div v-else class="controls-card">
        <div class="search-group">
          ...
          <input
            ...
            type="text"
            class="search-input"
            placeholder="Search movies by name, genre, or tag..."
          />
        </div>
      ...
      <SearchOverlay v-model="showSearch" :items="movies" @select="goToShow" />

      <div v-if="!loading && !error && movies.length > 0 && pagedMovies.length === 0" class="state state-no-match">
        <p>No shows found matching "**{{ query }}**". Try a broader search.</p>
      </div>

      <div v-else-if="!error" class="grid">
        <div class="card" v-for="m in pagedMovies" :key="m.id" @click="goToShow(m)">
          <div class="card-poster">
            <img :src="resolvedImage(m.image, m.name)" :alt="m.name" class="poster-img" width="300" height="450" />
            <div class="card-glow"></div>
          </div>
          <div class="card-info">
            <h3 class="card-title">{{ m.name }}</h3>
            <p class="card-meta">{{ m.tags || 'General' }}</p>
            <div class="actions">
              <button class="btn primary-action" @click.stop="book(m)">Book Seats</button>
              <button class="btn secondary-action" @click.stop="goToShow(m)">Details</button>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    fetchMovies() {
      ...
      // Simulating a backend call that returns show data
      axios.get('shows', { headers })
        .then(res => {
          const shows = Array.isArray(res.data) ? res.data : [];
          const excludeNames = new Set(['dune: part two', 'inside out 2']);
          const onlyMovies = shows
            .filter(s => (s.tags || '').toLowerCase().includes('movie'))
            .filter(s => !excludeNames.has((s.name || '').toLowerCase()));
          this.movies = onlyMovies.map(s => ({
            id: s.id,
            name: (s.name || '').trim(),
            tags: s.tags,
            image: s.image 
          ...
          // Always augment with classics (deduped by name) so posters are visible
          const classics = [
            { id: 550, name: 'Fight Club', tags: 'Movie', image: 'https://image.tmdb.org/t/p/w500/pB8BM7pdSp6B6Ih7QZ4DrQ3PmJK.jpg' },
            { id: 278, name: 'The Shawshank Redemption', tags: 'Movie', image: 'https://image.tmdb.org/t/p/w500/9cqNxx0GxF0bflZmeSMuL5tnGzr.jpg' },
            { id: 238, name: 'The Godfather', tags: 'Movie', image: 'https://image.tmdb.org/t/p/w500/e5iVtjkjM30znn86JsvkBYtvEo1.jpg' },
            { id: 240, name: 'The Godfather: Part II', tags: 'Movie', image: 'https://image.tmdb.org/t/p/w500/hek3koDUyRQk7FIhPXsa6mT2Zc3.jpg' },
            { id: 424, name: "Schindler's List", tags: 'Movie', image: 'https://image.tmdb.org/t/p/w500/sF1U4EUQS8YHUYjNl3pMGNIQyr0.jpg' },
            { id: 389, name: '12 Angry Men', tags: 'Movie', image: 'https://image.tmdb.org/t/p/w500/ow3wq89wM8qd5X7hWKxiRfsFf9C.jpg' },
          ];
          const existingNames = new Set(this.movies.map(m => (m.name || '').toLowerCase()));
          const toAdd = classics.filter(c => !existingNames.has(c.name.toLowerCase()));
          this.movies = [...this.movies, ...toAdd];
        })
        .catch(err => {
          console.error('Error loading movies', err);
          this.error = 'Error connecting to the server. Check your connection.';
        })
        .finally(() => {
          this.loading = false;
        });
    ...
    book(show) {
      // Always use the same logic as goToShow for booking
      this.goToShow(show);
    },
    noop() {},
    localAssetFor(name) {
    ...
    resolvedImage(url, name) {
      ...
      if (tmdb) return tmdb;
      // Resolve backend-provided image to a usable URL
      // Accept full URLs, otherwise treat as filename served from backend uploads
      if (url && typeof url === 'string' && url.length > 0) {
        const trimmed = url.trim();
        if (/^https?:\/\//i.test(trimmed)) return trimmed;
        // If looks like a filename (e.g., poster.jpg), build uploads URL
        return `uploads/${encodeURIComponent(trimmed)}`;
      }
      return this.placeholder;
    },
    // book(show) is now handled above with goToShow
    noop() {}
  }
};
</script>
